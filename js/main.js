...
const jobSearchFn = () => {
      ...
      .then((data) => {
        ...
        if (data == null || data.length == 0) {
          ...
          ul.innerHTML = data
            .map((jobs) => {
              return `
                <li class='jobs--list list--animation ${jobs.type.slice(
                  0,
                  4
                )}${jobs.type.slice(5, 9)}'>
                <section class="jobs">
                    <section class="img--section list--animation-2">
                       ...
                       <section class="info">
                        <h2><span class="job-title list--animation-3"  >${
                          jobs.title
                        }</span></h2>
                  
                  
                        
                        <span class="posted-time list--animation-3" >${time(
                          jobs.created_at
                        )}</span>
                        <article>
                           
                            <h3 class="job-type list--animation-3">${
                              jobs.type
                            }</h3>
                            <h3 class="job-location list--animation-3" >${
                              jobs.location
                            }</h3>
     ...
     <section class="selected--job">
                <section class="job--intro">
                    ...
                    <section class="job--info--section">
                        ...
                        <a href="${
                          dataObj.company_url
                        }" target="_blank" rel="noopener noreferrer"  class= 'site'>company site</a>
            ...
            <section class="jobs--overview">
                 <section class="job--description">
                  ...
                  <section>
                      <h3 id="time">${time(
                        dataObj.created_at
                      )}<span></span>.<span>${dataObj.type}</span></h3>
                ...
                <a href="${
                  dataObj.url
                }" target="_blank" rel="noopener noreferrer" class="site">apply now</a>
  ...
  const filterFn = (e) => {
    ...
    switch (attr) {
      case "fulltime":
        jobsArr.filter((jobs) => {
          jobs.classList.contains("FullTime")
      ...
      case "parttime":
        jobsArr.filter((jobs) => {
          jobs.classList.contains("PartTime")
            ? (jobs.style.display = "grid")
            : (jobs.style.display = "none");
        });
